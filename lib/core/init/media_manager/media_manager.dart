...
import '../../../ui/shared/widgets/dialogs/pick_imagesource_dialog.dart';

class MediaManager {
  final ImagePicker _picker = ImagePicker();

  Future<List<XFile>?> pickImageWithDialog(BuildContext context, {bool allowMultiPick = false}) async {
    final imageSource = await showImageSourceDialog(context);
    if (imageSource == null) {
      return null;
    }
    List<XFile>? images = await pickImage(imageSource: imageSource, pickMultiImage: allowMultiPick);
    if (images == null) {
      return null;
    }
    return images;
  ...
  Future<ImageSource?> showImageSourceDialog(BuildContext context) async {
    ImageSource? selectedImageSource =
        await showDialog(context: context, builder: (_) => PickImageSourceDialog());
    return selectedImageSource;
  }

  Future<List<XFile>?> pickImage(
      {bool pickMultiImage = true, ImageSource? imageSource = ImageSource.gallery}) async {
