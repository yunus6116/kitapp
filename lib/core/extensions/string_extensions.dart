...
extension StringExtensions on String {
  ...
  Future<String?> get launchWebsite async {
    if (await canLaunchUrl(Uri.parse(this))) {
      ...
      final SnackBar snackBar =
          SnackBar(content: Text('Could not launch $this'));
  ...
  String storageUrlToDownloadUrl({String size = ''}) {
    String downloadUrlBase = Environment.envStorageReference;
    String extension = split('.').last;
    String sizePart = (size == '') ? '' : '@$size';
    String downloadUrl =
        '$downloadUrlBase${Uri.encodeComponent(substring(1).split('.')[0])}$sizePart.$extension?alt=media';
    return downloadUrl;
  }

  static String slugify(String string) {
    Map<String, String> charMap = json
        .decode(
        ...
        .cast<String, String>();
    List<String> sourceChars = charMap.keys.toList();

    String replacedString =
        string.replaceAllMapped(RegExp('[${sourceChars.join()}]'), (Match m) {
      return charMap[m.group(0)]!;
    });
    String decontaminatedString =
        replacedString.replaceAll(RegExp("[^\\w\\s\$*_+~.()'\"!-:@]"), '');
    String trimmedString = decontaminatedString.trim();
    String kebapCasedString = trimmedString.replaceAll(RegExp("[-\\s]+"), '-');
    String lowercasedString = kebapCasedString.toLowerCase();
